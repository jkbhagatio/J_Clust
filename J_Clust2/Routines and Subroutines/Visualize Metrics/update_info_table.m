...
update_cluster_info = cell(13,4);

update_cluster_info{1,1} = length(ts);
update_cluster_info{1,2} = length(overlaps);
...
for i = 2:length(unit_pts)+1 %entries in table corresponding to units
    update_cluster_info{i,1} = length(unit_pts{i-1});
    update_cluster_info{i,2} = length(intersect(unit_pts{i-1}, overlaps)); 
    update_cluster_info{i,3} = length(shared_spikes{i-1});
    update_cluster_info{i,4} = num2str(shared_cluster{i-1}); 
end

cluster_info.Data = update_cluster_info;
end